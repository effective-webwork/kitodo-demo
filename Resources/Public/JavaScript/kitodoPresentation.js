...
function tocToggler() {
    ...
    const toggles = document.querySelectorAll(".tx-dlf-tableofcontents li.tx-dlf-tableofcontents-submenu > .toc-entry > .toggle");

    toggles.forEach(toggle => {
        toggle.addEventListener("click", (e) => {
            e.stopPropagation(); // prevent bubbling

            // find the parent <li> with a submenu
            const li = toggle.closest("li.tx-dlf-tableofcontents-submenu");
            if (!li) return;

            // toggle classes
            const isClosed = li.classList.contains("tx-dlf-tableofcontents-no");
            if (isClosed) {
                li.classList.remove("tx-dlf-tableofcontents-no");
                li.classList.add("tx-dlf-tableofcontents-active");
                toggle.textContent = "−"; // open
            } else {
                li.classList.remove("tx-dlf-tableofcontents-active");
                li.classList.add("tx-dlf-tableofcontents-no");
